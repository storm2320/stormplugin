...
def restoreLocal(type):
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]Storm schliessen[/COLOR] oder das [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Storm schliessen[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)
...
def restoreExternal(type):
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder das [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profile neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Storm schliessen[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('KODI schliessen', '[COLOR %s]Du bist dabei Storm zu schliessen' % COLOR2, 'Willst du fortfahrene?[/COLOR]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, Storm schliessen[/COLOR][/B]')
	if choice == 1:
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]WARNUNG: Manchmal kann beim Profil neu laden Storm absturzen.  Solange das Profil neu geladen wird, bitte keine Tasten drucken![/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
