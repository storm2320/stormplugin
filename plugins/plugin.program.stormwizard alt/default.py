...
from resources.libs import extract, downloader, notify, debridit, traktit, skinSwitch, uploadLog, yt, wizard as wiz
...
ICONADVANCED     = uservar.ICONADVANCED if not uservar.ICONADVANCED == 'http://' else ICON
...
ICONPAY          = uservar.ICONPAY if not uservar.ICONPAY == 'http://' else ICON
...
def index():
	...
	else: addDir('Kein Build installiert', 'builds', themeit=THEME4)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addDir ('[B]Builds installieren[/B]'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	if not ADDONFILE == 'http://': addDir ('[B]Addon/Repo Installer[/B]' ,'addons', icon=ICONADDONS, themeit=THEME1)
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('[B]Apk Installer[/B]' ,'apk', icon=ICONAPK, themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addDir ('Advanced Settings',            'advancedsetting',  icon=ICONADVANCED, themeit=THEME1)
	addDir ('Backup/Wiederherstellung'     ,'maint', 'backup',   icon=ICONBACKUP, themeit=THEME1)
	addFile ('Internet Test' ,'speed', icon=ICONSPEED, themeit=THEME1)
	...
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addDir ('Einstellungen: Daten behalten'     ,'savedata', icon=ICONSETTINGS,     themeit=THEME3)
	addFile('Einstellungen: Wizard'      ,'settings', icon=ICONSETTINGS, themeit=THEME3)
	addDir ('[B]Cleaning Tools[/B]'       ,'maint', 'clean',  icon=ICONMAINT, themeit=THEME3)
	if DEVELOPER == 'true': addDir('Entwickler Menu','developer', icon=ICONSETTINGS, themeit=THEME3)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		addDir ('Einstellungen: [COLOR white]Daten behalten[/COLOR]'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addDir ('Einstellungen: [COLOR white]Daten behalten[/COLOR]',       'savedata', icon=ICONSAVE,     themeit=THEME3)
		addFile('Information: [COLOR white]Build Inhalt[/COLOR]',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		if not preview == "http://": addFile('Video: [COLOR white]Build Vorschau[/COLOR]', 'buildpreview', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		...
		if warning == True:
			addFile('[I][COLOR red]Build fuer Kodi Version [COLOR lime]%s[/COLOR] (installiert: %s)[/COLOR][/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile(wiz.sep('INSTALLIEREN'), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('[COLOR lime]Frische Installation[/COLOR]'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
			...
			if len(match) > 0:
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						do        = ['Addon/Repo oeffnen', 'Addon/Repo entfernen']
						selected = DIALOG.select("[COLOR %s]Addon/Repo bereits installiert , was moechtest du machen?[/COLOR]" % COLOR2, do)
						if selected == 0:
...
def maintMenu(view=None):
	...
	addDir ('[B]Advanced Settings[/B]',            'advancedsetting',  icon=ICONADVANCED, themeit=THEME1)
	if view == "tweaks" or SHOWMAINT == 'true': 
		if not ADVANCEDFILE == 'http://' and not ADVANCEDFILE == '':
			addDir ('Advanced Settings',            'advancedsetting',  icon=ICONADVANCED, themeit=THEME1)
		else: 
			if os.path.exists(ADVANCED):
				addFile('Zeige aktuelle AdvancedSettings.xml',   'currentsettings', icon=ICONADVANCED, themeit=THEME3)
				addFile('Entferne aktuelle AdvancedSettings.xml', 'removeadvanced',  icon=ICONADVANCED, themeit=THEME3)
			addFile('[B]Advanced Settings Konfigurator[/B]',    'autoadvanced',    icon=ICONADVANCED, themeit=THEME3)
	addDir ('[B]Backup/Wiederherstellung[/B]'     ,'maint', 'backup',   icon=ICONBACKUP, themeit=THEME1)
	if view == "backup" or SHOWMAINT == 'true':
		addFile('Reinige Backup Ordner',        'clearbackup',     icon=ICONBACKUP,   themeit=THEME3)
		addFile('Backup Pfad: [COLOR %s]%s[/COLOR]' % (COLOR2, MYBUILDS),'settings', 'Maintenance', icon=ICONBACKUP, themeit=THEME3)
		addFile('[Backup]: Build',               'backupbuild',     icon=ICONBACKUP,   themeit=THEME3)
		addFile('[Backup]: GuiFix',              'backupgui',       icon=ICONBACKUP,   themeit=THEME3)
		addFile('[Wiederherstellung]: Lokales Build',         'restorezip',      icon=ICONBACKUP,   themeit=THEME3)
		addFile('[Wiederherstellung]: Lokaler GuiFix',        'restoregui',      icon=ICONBACKUP,   themeit=THEME3)
		addFile('[Wiederherstellung]: Externes Build',      'restoreextzip',   icon=ICONBACKUP,   themeit=THEME3)
		addFile('[Wiederherstellung]: Externer GuiFix',     'restoreextgui',   icon=ICONBACKUP,   themeit=THEME3)
	addDir ('[B]System Log[/B]'     ,'maint', 'misc',   icon=ICONLOG, themeit=THEME1)
	if view == "misc" or SHOWMAINT == 'true': 
		addFile('Upload Kodi.log [COLOR red](funktioniert noch nicht)[/COLOR]',                'uploadlog',       icon=ICONLOG, themeit=THEME3)
		addFile('Zeige Fehler in der Log: %s' % (errorsfound), 'viewerrorlog',    icon=ICONLOG, themeit=THEME3)
		addFile('Zeige Log Datei',                  'viewlog',         icon=ICONLOG, themeit=THEME3)
		addFile('Zeige Wizard Log Datei',           'viewwizlog',      icon=ICONLOG, themeit=THEME3)
		addFile('Reinige Wizard Log Datei%s' % wizlogsize,'clearwizlog',     icon=ICONLOG, themeit=THEME3)
	addDir ('[B]Reinigung Tools[/B]'       ,'maint', 'clean',  icon=ICONCLEAN, themeit=THEME1)
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Komplette Reinigung: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONCLEAN, themeit=THEME3)
		addFile('Cache reinigen: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache)     ,'clearcache',      icon=ICONCLEAN, themeit=THEME3)
		addFile('Packages reinigen: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack)   ,'clearpackages',   icon=ICONCLEAN, themeit=THEME3)
		addFile('Thumbnails reinigen: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),'clearthumb',      icon=ICONCLEAN, themeit=THEME3)
		addFile('Alte Thumbnails reinigen', 'oldThumbs',      icon=ICONCLEAN, themeit=THEME3)
		addFile('Crash Logs reinigen',               'clearcrash',      icon=ICONCLEAN, themeit=THEME3)
		addFile('Datenbank reinigen',                'purgedb',         icon=ICONCLEAN, themeit=THEME3)
		addFile('[B][COLOR red]Frischer Start[/COLOR][/B] [COLOR white]- alle Daten werden entfernt![/COLOR]',                    'freshstart',      icon=ICONCLEAN, themeit=THEME3)
	addDir ('[I]<< Zum Hauptmenu[/I]', icon=ICONMAINT, themeit=THEME3)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('[B]Auto Reinigung[/B]', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	addFile('[I][B]Auto Reinigung beim Start:[/I][/B] %s' % autoclean.replace('true',on).replace('false',off) ,'togglesetting', 'autoclean',   icon=ICONMAINT, themeit=THEME3)
	if autoclean == 'true':
	...
	addFile('[B]Reinige Video Cache[/B]', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	addFile('[I][B]Video Cache mit einbeziehen beim Cache reinigen:[/I][/B] %s' % includevid.replace('true',on).replace('false',off), 'togglecache', 'includevideo', icon=ICONMAINT, themeit=THEME3)
	if includevid == 'true':
		...
		addFile(' [COLOR lime]Aktiviere alle Video Addons[/COLOR]', 'togglecache', 'true', icon=ICONMAINT, themeit=THEME3)
		addFile(' [COLOR red]Deaktiviere alle Video Addons[/COLOR]', 'togglecache', 'false', icon=ICONMAINT, themeit=THEME3)
	addDir ('[I]<< Zum Hauptmenu[/I]', icon=ICONMAINT, themeit=THEME3)
	setView('files', 'viewType')
...
def advancedWindow(url=None):
	if not ADVANCEDFILE == 'http://':
		...
		addFile('[B]Advanced Settings Konfigurator[/B]', 'autoadvanced', icon=ICONADVANCED, themeit=THEME1)
		if os.path.exists(ADVANCED): 
			addFile('Zeige aktuelle AdvancedSettings.xml', 'currentsettings', icon=ICONADVANCED, themeit=THEME3)
			addFile('Entferne aktuelle AdvancedSettings.xml', 'removeadvanced',  icon=ICONADVANCED, themeit=THEME3)
...
def writeAdvanced(name, url):
	ADVANCEDWORKING = wiz.workingURL(url)
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du deine aktuelle AdvancedSettings.xml [COLOR %s]%s[/COLOR] ersetzen?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR lime]JA, Ersetzen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, Abbrechen[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die AdvancedSettings.xml fuer [COLOR %s]%s[/COLOR] downloaden und installieren?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR lime]Installieren[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
		...
		if choice == 1:
			...
			f.write(file)
			f.close()
			DIALOG.ok(ADDONTITLE, '[COLOR %s]AdvancedSettings.xml wurde erfolgreich geschrieben.  Klicke [COLOR lime][B]OK[/COLOR][/B] um Storm zum schliessen zu zwingen.[/COLOR]' % COLOR2)
			wiz.killxbmc(True)
...
def saveMenu():
	...
	addDir ('Behalte Trakt Daten',               'trakt',                icon=ICONTRAKT, themeit=THEME1)
	addDir ('Behalte Real Debrid',              'realdebrid',           icon=ICONREAL,  themeit=THEME1)
	addFile('Importiere gespeichete Daten',              'managedata', 'import', icon=ICONSAVE,  themeit=THEME1)
	addFile('Exportiere gespeicherte Daten',              'managedata', 'export', icon=ICONSAVE,  themeit=THEME1)
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]Storm schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR lime]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR red]Storm schliessen[/COLOR][/B]")
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		...
		if yes:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Der GuiFix wurde installiert.  Willst du das Profil neu laden oder Storm schliessen?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Storm schliessen[/COLOR][/B]")
			if todo == 1: wiz.reloadFix()
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Um die Anderungen zu speichern musst du Storm jetzt schliessen, klicke OK um Storm zu schliessen[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]Storm schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Storm schliessen[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
...
def manageSaveData(do):
	if do == 'import':
		...
		dir = [traktit.TRAKTFOLD, debridit.REALFOLD]
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du Storm resetten" % COLOR2, "auf standard Einstellungen", "bevor du das lokale Backup installierst?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du Storm resetten" % COLOR2, "auf standard Einstellungen", "bevor du [COLOR %s]%s[/COLOR] installierst?" % (COLOR1, install), nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du Storm resetten" % COLOR2, "auf standard Einstellungen?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
...
def viewIP():
	...
	setView('files', 'viewType')
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.program.kus-allinone-wizard/speedtest.py")')
